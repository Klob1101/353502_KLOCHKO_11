...
from books.models import UserSession
...
class TimezoneMiddleware:
    ...
    def __call__(self, request):
        if request.user.is_authenticated:
            # Use user's profile timezone if set
            try:
                tzname = request.user.customer.timezone
            except:
                tzname = None
        else:
            # Fall back to session timezone for anonymous users
